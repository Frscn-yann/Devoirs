#Exercice 1
df = read.csv(file = "L:/BUT/SD/Promo 2023/yfrascone/Programmation Stat/TD/TD6/NBA2014_2015.csv", 
              sep = ",",
              header = TRUE, 
              dec = ".")
# redéfinir l'emplacement du fichier, séparateur et dec mauvais, header en TRUE.
nrow(df) #Changer en nrow(df) et non pas nrow(nba).
ncol(df) #Changer en ncol(df) et non pas nrcol(nba).
colnames(df) # Rajouter un s a colname(df).
df$PERIOD = as.factor(df$PERIOD) #PERIOD en MAJ et remplacer <- par =.
df$PTS_TYPE = as.factor(df$PTS_TYPE) #Rajouter un _.
df$SHOOTER = as.factor(df$SHOOTER) # mettre SHOOTER en MAJ.
...
df$GAME_RESULT = NULL # Rajouter un L 
       
#création d'un objet sans la première colonne GAME_ID
df2 = df[ , -1] #changer de coter -1
